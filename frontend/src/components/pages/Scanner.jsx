...
function analyzeProduct(product, t) {
  ...
  if (product.sugar != null) {
    if (product.sugar <= 1) qualities.push({ icon: "🍬", label: t("sugar"), desc: t("qualSugarLow"), value: `${product.sugar.toFixed(1)}g` });
    else if (product.sugar <= 5) qualities.push({ icon: "🍬", label: t("sugar"), desc: t("qualSugarOk"), value: `${product.sugar.toFixed(1)}g` });
    else if (product.sugar <= 10) defauts.push({ icon: "🍬", label: t("sugar"), desc: t("defSugarMed"), value: `${product.sugar.toFixed(1)}g` });
    else defauts.push({ icon: "🍬", label: t("sugar"), desc: t("defSugarHigh"), value: `${product.sugar.toFixed(1)}g` });
  }
  if (product.saturatedFat != null) {
    if (product.saturatedFat <= 0.5) qualities.push({ icon: "🧈", label: t("saturatedFat"), desc: t("qualSatFatLow"), value: `${product.saturatedFat.toFixed(1)}g` });
    else if (product.saturatedFat <= 2) qualities.push({ icon: "🧈", label: t("saturatedFat"), desc: t("qualSatFatOk"), value: `${product.saturatedFat.toFixed(1)}g` });
    else if (product.saturatedFat <= 5) defauts.push({ icon: "🧈", label: t("saturatedFat"), desc: t("defSatFatMed"), value: `${product.saturatedFat.toFixed(1)}g` });
    else defauts.push({ icon: "🧈", label: t("saturatedFat"), desc: t("defSatFatHigh"), value: `${product.saturatedFat.toFixed(1)}g` });
  }
  if (product.salt != null) {
    if (product.salt <= 0.1) qualities.push({ icon: "🧂", label: t("salt"), desc: t("qualSaltLow"), value: `${product.salt.toFixed(1)}g` });
    else if (product.salt <= 0.6) qualities.push({ icon: "🧂", label: t("salt"), desc: t("qualSaltOk"), value: `${product.salt.toFixed(1)}g` });
    else if (product.salt <= 1.5) defauts.push({ icon: "🧂", label: t("salt"), desc: t("defSaltMed"), value: `${product.salt.toFixed(1)}g` });
    else defauts.push({ icon: "🧂", label: t("salt"), desc: t("defSaltHigh"), value: `${product.salt.toFixed(1)}g` });
  }
  ...
  if (product.fiber != null) {
    if (product.fiber >= 6) qualities.push({ icon: "🥦", label: t("fiber"), desc: t("qualFiberHigh"), value: `${product.fiber.toFixed(1)}g` });
    else if (product.fiber >= 3) qualities.push({ icon: "🥦", label: t("fiber"), desc: t("qualFiberOk"), value: `${product.fiber.toFixed(1)}g` });
    else if (product.fiber > 0) defauts.push({ icon: "🥦", label: t("fiber"), desc: t("defFiberLow"), value: `${product.fiber.toFixed(1)}g` });
  }
  if (product.proteins != null) {
    if (product.proteins >= 15) qualities.push({ icon: "💪", label: t("proteins"), desc: t("qualProtHigh"), value: `${product.proteins.toFixed(1)}g` });
    else if (product.proteins >= 5) qualities.push({ icon: "💪", label: t("proteins"), desc: t("qualProtOk"), value: `${product.proteins.toFixed(1)}g` });
  }
...
function ScoreCircle({ score, lang, ns }) {
  ...
  return (
    <div className="score-group">
      {ns && <div className="nutri-letter-only" style={{ background: NUTRI_BG[ns] }}>{ns.toUpperCase()}</div>}
  ...
  return (
    <div className="product-result">
      ...
      <div className="analysis-section">
        ...
        {altLoading && (
          <div className="alt-loading"><div className="alt-spinner" /><p>Recherche en cours sur Marjane & Aswak Assalam…</p></div>
        )}
        ...
        {!altLoading && alternatives.length > 0 && (
          <div className="alt-grid">
            {alternatives.map((alt, i) => (
              <a key={i} href={alt.url} target="_blank" rel="noopener noreferrer" className="alt-card">
                {alt.image ? <img src={alt.image} alt={alt.titre} className="alt-image" /> : <div className="alt-image alt-image-placeholder">🛒</div>}
                <div className="alt-body">
                  <p className="alt-titre">{alt.titre}</p>
                  <div className="alt-footer">
                    <span className="alt-nutri-badge" style={{ background: NUTRI_BG[alt.nutriScore?.toLowerCase()] || "#ccc" }}>{alt.nutriScore}</span>
...
export default function Scanner() {
  ...
  const handleScan = async (code) => {
    ...
    setAlternatives([]);
    try {
      const res = await apiFetch(`/api/product/${code}?lang=${lang}`);
      ...
      if (data.nutriScore) {
        setAltLoading(true);
        apiFetch(`/api/scrape/alternatives?q=${encodeURIComponent(data.name)}&currentScore=${data.nutriScore}`)
          .then(r => r.json())
  ...
  return (
    <div className="scanner-app">
      ...
      <div className="scanner-body">
        ...
        {product && <ProductResult product={product} alternatives={alternatives} altLoading={altLoading} />}
      </div>
    </div>
  );
}